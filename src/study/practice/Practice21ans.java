...
//
		...
		while(true) {
			...
			isDuplicated = false; //중복이냐? true-> 중복O false -> 중복X
			//중ㄷ복 여부 사전 false로 초기화
...
//		for(int i=0;i<lotto.length;i++) {			//중복되었을때 안에서 다시 뽑은값이 현재 j인덱스값의 이전인값이면 검출 불가
//			for(int j=0;j<i;j++) {
//				if(lotto[i]==lotto[j]) {
...
//		for(int i=0;i<lotto.length;i++) {			//인덱스 이전의 중복이 되면 못 막음
//			for(int j=0;j<i;j++) {
//				if(lotto[i]==lotto[j]) {
//					lotto[i]=random.nextInt(1,46);
//				}
